...
export async function generateMetadata({ params }: any) {
    ...
    try {
        ...
        return {
            ...
            description: desc,
            alternates: {
                canonical: `/${locale}/bolgeler/${slug}`
            }
        };
    } catch (e) {
        return {};
...
export default async function RegionPage({ params }: any) {
    ...
    // Extract region name from title (e.g., "Muratpaşa Çilingir" -> "Muratpaşa")
                ...
                <div className="container" style={{ maxWidth: '800px', margin: '0 auto', padding: '0 20px' }}>
                    ...
                    <div style={{ marginBottom: '40px' }}>
                        ...
                        <p style={{ fontSize: '1.15rem', lineHeight: '1.8', color: '#444' }}>
                            Antalya <strong>{regionName}</strong> bölgesi için acil çilingir hizmeti talebiniz alındıktan sonra, size en yakın konumda bulunan motorlu mobil çilingir servisimiz ortalama 15 dakika içerisinde adresinize ulaşır. Yalnızca kapı açma değil, kilit değişimi, kasa açma, oto çilingir ve güvenlik devriye hizmetlerimizle yanınızdayız.
                        </p>
                    </div>
                    ...
                    <div style={{ padding: '40px', background: 'white', borderRadius: '16px', textAlign: 'center', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', border: '1px solid rgba(0,0,0,0.05)' }}>
                        <h3 style={{ marginBottom: '15px', fontSize: '1.5rem', color: '#111' }}>{regionName} Çilingir Çağırın</h3>
                        <p style={{ marginBottom: '24px', color: '#666' }}>7/24 kesintisiz hizmet için hemen bizi arayın.</p>
